...
const AdminDetailBox = () => {
    return (
        <>
            <div className='grid xs:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xs:px-6 md:px-8 lg:px-4 xl:px-16 2xl:px-24 xs:gap-4 md:gap-6 lg:gap-4 xl:gap-5 w-full h-auto overflow-hidden'>
                {
                    detailCards.map((items, index) => {
                        const { icon, iconText, bgIconColor, title, description, count, bgColor } = items;
                        return (
                            <div key={index} style={{ backgroundColor: bgColor }} className='flex items-start justify-center flex-col px-4 py-5 gap-y-4 rounded-lg shadow-sm text-amber-50 duration-100'>
                                ...
                                <h1 className='w-full text-[1.1em] font-serif'>
                                    <CountUp
                                        end={count}
                                        separator=','
                                        suffix='+'
