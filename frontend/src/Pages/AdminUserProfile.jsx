...
const AdminUserProfile = () => {
  ...
  return (
    <>
      <div className='flex flex-col justify-start xs:px-3 sm:px-6 md:px-5 lg:px-6 xl:px-8 w-full h-full duration-150'>
        ...
        <div className='grid xs:grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-y-6 sm:gap-x-5 lg:gap-x-4 w-full h-auto'>
          {
            getAlluser.map((items, index) => {
              const { _id, firstname, lastname, email, address } = items;
              return (
                <div key={index} className='header-theme ProductComp flex flex-col px-4 py-4 w-full h-fit text-[0.96em] rounded-md xs:order-2 md:order-1 '>
                  ...
                  <div className='flex flex-col gap-y-4 mt-7 w-full h-auto'>

                    <div className='custom-input admin-form-border flex gap-x-1'>
                      <p>{`${firstname}`}</p>
                      <p>{`${lastname}`}</p>
                    </div>
                  ...
                  <div className='flex pt-5 gap-x-3'>
                    <NavLink to={`/admin/alluser/${_id}/edit`} >
                      <button className='px-[10px] py-[5px] text-[0.8em] uppercase hover:bg-blue-800'>
                        Edit
                      </button>
                    </NavLink>
                    <button onClick={() => deleteUserData(_id)} className='px-[8px] py-[5px] text-[0.8em] uppercase bg-red-600 hover:bg-red-400 duration-100 bg-gradient-to-b from-none hover:bg-gradient-to-b from-none'>
                      Delete
                    </button>
                  </div>
                </div>
