...
const SignUp = () => {
    ...
    const handleSignUpForm = async (e) => {
        ...
        try {
            const url = 'https://apna-store-mern-backend.vercel.app/auth/signup';
            ...
            const { success, message, jwtToken, firstname, lastname, email, address } = data;
            if (success) {
    ...
    return (
        <div className='filter-sec-theme flex items-center justify-center py-28 w-full min-h-screen duration-150'>
            <form onSubmit={handleSignUpForm} className='contact-form  ProductComp flex flex-col items-center justify-center py-8 w-full xs:px-[8px] sm:px-[21px] lg:px-[25px] xs:w-[405px] sm:w-[400px] lg:w-[460px] rounded-md xs:text-[0.98em] xl:text-[0.94em] text-amber-50'>
                ...
                <div className='flex items-center justify-center flex-col mt-9 xs:gap-y-3 w-full xs:text-[1em] md:text-[1em]'>
                    <button type='submit' className='w-full py-[10px] font-medium rounded-md bg-none hover:bg-none bg-[#db319d] hover:bg-[#a41570]'>
                        Sign Up
                    </button>
                    <p className='text-[0.9em]'>Already have an account ?</p>
                    <NavLink to='/login' className='w-full'>
                        <button className='w-full py-[10px] font-medium rounded-md bg-none hover:bg-none text-gray-900 bg-[#F9FAFB] hover:bg-[#bfc3c7]'>
                            Login
                        </button>
                    </NavLink>
                </div>
